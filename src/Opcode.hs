...
decodeOpcode w = (Opcode w mnemonic addressMode)
  where
    (mnemonic, addressMode) = case w of
      ...
      0x61 -> (ADC, IdxInd)      ; 0x71 -> (ADC, IndIdx)      ; 0x29 -> (AND, Immediate)
      0x25 -> (AND, ZeroPage)    ; 0x35 -> (AND, ZeroPageX)   ; 0x2D -> (AND, Absolute)
      0x3D -> (AND, AbsoluteX)   ; 0x39 -> (AND, AbsoluteY)   ; 0x21 -> (AND, IdxInd)
      0x31 -> (AND, IndIdx)      ; 0x0A -> (ASL, Accumulator) ; 0x06 -> (ASL, ZeroPage)
      ...
      0xC1 -> (CMP, IdxInd)      ; 0xD1 -> (CMP, IndIdx)      ; 0xE0 -> (CPX, Immediate)
      ...
      0x5D -> (EOR, AbsoluteX)   ; 0x59 -> (EOR, AbsoluteY)   ; 0x41 -> (EOR, IdxInd)
      0x51 -> (EOR, IndIdx)      ; 0xE6 -> (INC, ZeroPage)    ; 0xF6 -> (INC, ZeroPageX)
      ...
      0xB9 -> (LDA, AbsoluteY)   ; 0xA1 -> (LDA, IdxInd)      ; 0xB1 -> (LDA, IndIdx)
      ...
      0x19 -> (ORA, AbsoluteY)   ; 0x01 -> (ORA, IdxInd)      ; 0x11 -> (ORA, IndIdx)
      ...
      0xF9 -> (SBC, AbsoluteY)   ; 0xE1 -> (SBC, IdxInd)      ; 0xF1 -> (SBC, IndIdx)
      0x38 -> (SEC, Implied)     ; 0xF8 -> (SED, Implied)     ; 0x78 -> (SEI, Implied)
      0x85 -> (STA, ZeroPage)    ; 0x95 -> (STA, ZeroPageX)   ; 0x8D -> (STA, Absolute)
      0x9D -> (STA, AbsoluteX)   ; 0x99 -> (STA, AbsoluteY)   ; 0x81 -> (STA, IdxInd)
      0x91 -> (STA, IndIdx)      ; 0x86 -> (STX, ZeroPage)    ; 0x96 -> (STX, ZeroPageY)
      ...
      0xAF -> (LAX, Absolute)    ; 0xBF -> (LAX, AbsoluteY)   ; 0xA3 -> (LAX, IdxInd)
      0xB3 -> (LAX, IndIdx)      ; 0x87 -> (SAX, ZeroPage)    ; 0x97 -> (SAX, ZeroPageY)
      0x8F -> (SAX, Absolute)    ; 0x83 -> (SAX, IdxInd)      ; 0xEB -> (SBC, Immediate)
      0xC7 -> (DCP, ZeroPage)    ; 0xD7 -> (DCP, ZeroPageX)   ; 0xCF -> (DCP, Absolute)
      0xDF -> (DCP, AbsoluteX)   ; 0xDB -> (DCP, AbsoluteY)   ; 0xC3 -> (DCP, IdxInd)
      0xD3 -> (DCP, IndIdx)      ; 0xE7 -> (ISC, ZeroPage)    ; 0xF7 -> (ISC, ZeroPageX)
      0xEF -> (ISC, Absolute)    ; 0xFF -> (ISC, AbsoluteX)   ; 0xFB -> (ISC, AbsoluteY)
      0xE3 -> (ISC, IdxInd)      ; 0xF3 -> (ISC, IndIdx)      ; 0x27 -> (RLA, ZeroPage)
      0x37 -> (RLA, ZeroPageX)   ; 0x2F -> (RLA, Absolute)    ; 0x3F -> (RLA, AbsoluteX)
      0x3B -> (RLA, AbsoluteY)   ; 0x23 -> (RLA, IdxInd)      ; 0x33 -> (RLA, IndIdx)
      0x67 -> (RRA, ZeroPage)    ; 0x77 -> (RRA, ZeroPageX)   ; 0x6F -> (RRA, Absolute)
      0x7F -> (RRA, AbsoluteX)   ; 0x7B -> (RRA, AbsoluteY)   ; 0x63 -> (RRA, IdxInd)
      0x73 -> (RRA, IndIdx)      ; 0x07 -> (SLO, ZeroPage)    ; 0x17 -> (SLO, ZeroPageX)
      0x0F -> (SLO, Absolute)    ; 0x1F -> (SLO, AbsoluteX)   ; 0x1B -> (SLO, AbsoluteY)
      0x03 -> (SLO, IdxInd)      ; 0x13 -> (SLO, IndIdx)      ; 0x47 -> (SRE, ZeroPage)
      0x57 -> (SRE, ZeroPageX)   ; 0x4F -> (SRE, Absolute)    ; 0x5F -> (SRE, AbsoluteX)
      0x5B -> (SRE, AbsoluteY)   ; 0x43 -> (SRE, IdxInd)      ; 0x53 -> (SRE, IndIdx)
      0x0B -> (ANC, Immediate)   ; 0x2B -> (ANC, Immediate)   ; 0x4B -> (ALR, Immediate)
      0x6B -> (ARR, Immediate)   ; 0x8B -> (XAA, Immediate)   ; 0x93 -> (AHX, IndIdx)
